...
class TestCompressServer(unittest.TestCase):
    ...
    def test_transform_method_directly(self):
        ...
        result = self.etl_server.transform(
            test_data, "test.txt", '{"compression":"bz2"}'
        )
        ...
        result = self.etl_server.transform(
            compressed_data, "test.txt", '{"mode":"decompress"}'
        )
        ...
        result = self.etl_server.transform(
            bz2_compressed, "test.txt", '{"mode":"decompress","compression":"bz2"}'
        )
        self.assertEqual(result, test_data)
